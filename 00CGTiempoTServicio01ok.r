...
## CODIGO DE GRAFICA 
##
## TOTAL DE USOS POR TIEMPO
## VARIABLES: x:SEMANA, y:USO ACUMULADO: TIPO.BUS
## 
#########################################
...
## LINUX
mesesf <- c("/home/rjnogales/Dropbox/10.DRIVE/R.SHINY/data/dmes2021.02.Rds",
            "/home/rjnogales/Dropbox/10.DRIVE/R.SHINY/data/dmes2021.03.Rds",
...
udata <- data %>%
  ...
  filter(DIA %in% listaDia) %>%
  filter(TDIA %in% listaTDia) %>%
  filter(HORA %in% listaHora) %>%
  filter(CUARTO %in% listaCuarto) %>%
  
  # ORGANIZAR INFORMACION
  select(ESTACION,TSERVICIO,TBUS,TDIA,SEMANA,DIA,HORA,CUARTO,USOA) %>%
  arrange(ESTACION,TSERVICIO,TBUS,TDIA,SEMANA,DIA,HORA,CUARTO) %>%
  mutate(CUARTO = as.integer(CUARTO)) %>%
  
  ## GRUPO PARA ACUMULADO POR TIEMPO
  group_by(TDIA,TSERVICIO,SEMANA,DIA,HORA,CUARTO) %>%
  summarise(STiempo = sum(USOA)) %>%
  ungroup() 
...
#   mutate(ESTACIONN = NULL) %>%
  ...
  # ORGANIZAR EJE X: DIA+HORA+CUARTO
  mutate(HORA = ifelse(HORA < 10,
                       str_c("0",as.character(HORA)),
                       HORA)) %>%
  mutate(TIEMPO.X = str_c(DIA,".",HORA,".",CUARTO))
...
tic("Grafica I")
ggplot(data = vdata,aes(x = factor(TIEMPO.X),
                        y =  STiempo,
                        fill = TIPO.SERVICIO
                        )
       ) +
  ...
  # geom_text(aes(label = formatC(STiempo,
  #                               format = "f",
  #                               big.mark = ".",
  #                               digits = 0)),
  ...
  ## TITULOS EJES
  ggtitle(str_c("TOTAL DE USOS POR TIEMPO POR DIA TIPO & SERVICIO",
                ""
  ...
  # CUADROS 
  facet_wrap( TIPO.DIA ~ TIPO.SERVICIO,
             scales = "free_y",
             labeller = label_both,
             ncol = 3
             ) 
